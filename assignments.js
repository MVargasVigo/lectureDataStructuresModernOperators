...
const logBookTheme = function (title) {
  ...
  if (normalizedTitle.startsWith('computer')) {
    ...
    normalizedTitle.includes('operating') === false
  ) {
    console.log(
...
const logBookCategories = function (string) {
  const categoryArray = string.split(';');
...
const getKeywordsAsString = function (books) {
  ...
  for (let book of books) {
    const keywordsArray = book.keywords;
    for (let i = 1; i <= keywordsArray.length; i++) {
      if (keywordsArray[i] !== keywordsArray[i - 1]) {
        arrayOfUniques.push(keywordsArray[i]);
      }
    }
    const keywordStrings = arrayOfUniques.join(';');
...
const logBookChapters = function (bookChapters) {
  for (let chapter of bookChapters) {
    const chapterName = chapter[0].padEnd(15, '_');
    const chapterInfo = chapterName + ' ' + chapter[1];
